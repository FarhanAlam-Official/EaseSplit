...
export default function GenericErrorPage({ 
  ...
  const handleCopy = async () => {
    try {
      ...
      notifications.showSuccess("Error details copied")
    } catch {
      notifications.showError({ title: "Copy failed", description: "Please try again." })
    }
  }

  const handleReport = async () => {
    try {
      const subject = encodeURIComponent("EaseSplit Error Report")
      ...
      window.location.href = `mailto:support@easesplit.com?subject=${subject}&body=${body}`
      notifications.showInfo("Opening mail client")
    } catch {
      notifications.showError("Could not open mail client")
    }
  }
